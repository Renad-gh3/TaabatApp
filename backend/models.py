...
# ===========================
class User(Base):
    __tablename__ = "users"   # كله small عشان ما يصير تكرار Users / users
    ...
    role = Column(String, nullable=False)  # "Farmer" أو "Shopper"
...
# ===========================
#      FARM_FRUIT TABLE
#  (علاقة many-to-many بين Farm و Fruit)
# ===========================
...
class FruitImage(Base):
    ...
    ripeness = Column(String)      # مرحلة النضج (Ripe / Unripe / …)
    production = Column(String)    # إنتاجية المزرعة من هذه الفاكهة (نص مبدئياً)
    yield_amount = Column(String)  # المحصول/الكمية (نص مبدئياً)
    image_url = Column(String)     # رابط الصورة في التخزين
...
class NutritionalInfo(Base):
    ...
    image_id = Column(
        ...
        unique=True,  # لكل صورة nutrition واحد
    )
